...
def imshow(img, label, mean, std):
    ...
    i, j = 0, 0
    for k in range(16):
        img_ = np.transpose(img[k], axes=[1, 2, 0]) * std + mean
        i = k // 4
        j = k % 4
        l = label[k]
        ax[i][j].set_axis_off()
        ax[i][j].set_title(classes[l])
        ax[i][j].imshow(img_)
...
def run_training(model, optimizer, loss_function, device, num_epochs,
    ...
    for epoch in master_bar:
        ...
        if early_stopper:
            ...
            ## YOUR CODE HERE ##
            ####################
            if early_stopper.early_stop(epoch_val_acc, model):
                break
...
def plot(title, label, train_results, val_results, yscale='linear', save_path=None,
    ...
    if extra_pt:
        ...
        ## YOUR CODE HERE ##
        ####################
        plt.scatter(extra_pt[0], extra_pt[1], s=200, marker='x')
